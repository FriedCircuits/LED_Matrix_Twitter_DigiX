...
01-19-2014 - William Garrido
Finished coverting for DigiX and Twitter API 1.1
This requires a PHP script to grab the data from Twiiter with oAuth

ToDO:
Implement WiFi server to remotely change search query
Figure out why requires reset button after first powerup
*/
...
// Wifi parameters
//Set on Wifi chip via config webpage
...
char tweet[140];
char serverName[] = "Your PHP Twitter connection script";  // twitter URL
 ...
 */
 ...
 SerialUSB.println("WiFi Starting...");
  client.begin(9600);
  ...
  while (client.ready() != 1)
  {
    ...
    SerialUSB.println("WiFi not ready");
    delay(15000);
  }  
  SerialUSB.println("DigiX - Ready!");
  
  delay(2000);
  String address = client.server(8080);
...
void getTwitter(){
  ...
  if ((millis() > (delayTime + delayTwitter)) || (firstBoot)){
   ...
   //if (client.connect()) {
    ...
    client.println("HOST: www1.mcsg.us");
    client.println();
...
void wifi(){
  //test of wifi server so far not responding to request
  
  if ( client.serverRequest()){
      SerialUSB.print("Request for: ");
     SerialUSB.println(client.serverRequestPath());
     if(client.serverRequestPath()!="/")
       client.serverResponse("404 Not Found",404); 
      else{
       client.serverResponse("<html><body><h1>This is a test</h1></body></html>"); //defaults to 200
     }
